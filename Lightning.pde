...
void mouseClicked(){ //resets the background on click
  lightningBolt((int)(Math.random()*255), (int)(Math.random()*255), (int)(Math.random()*255));
...
//(honestly way more complicated than it needs to be)
  ...
  for(int i=0; i<numbersX.size()-1; i++){
    stroke(r, g, b);
    line(numbersX.get(i), 400, numbersX.get(i+1), sorted.get(i+1));
  }
}
